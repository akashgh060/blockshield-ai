...
async function startServer() {
  ...
  app.get('/api/ai/status', (req: Request, res: Response) => {
    res.json({
      ...
      provider: process.env.GEMINI_API_KEY ? 'Google Gemini Live' : 'Deterministic Mock Provider',
    });
  });
    ...
    try {
      ...
      if (!ai) {
        return res.status(503).json({
          error: 'GEMINI_API_KEY is not configured on server. Use Demo Mode or set key in Settings.',
        });
      }
...
Provide your response strictly in the following JSON format matching the schema:
{
  ...
  "defensiveConsiderations": "Actionable best practices (e.g. fresh addresses, coin control, PayJoin, Taproot)"
      ...
      const contextSummary = JSON.stringify({
        ...
        privacyScore: context?.privacyAnalysis?.overallRiskScore,
        riskLevel: context?.privacyAnalysis?.riskLevel,
        findings: context?.privacyAnalysis?.findings?.map((f: { title: string; severity: string; description: string }) => ({
          title: f.title,
          severity: f.severity,
...
Generate a thorough, defensive, structured privacy intelligence analysis.`;
      ...
      try {
        ...
        return res.json({
          structuredResponse: {
            ...
            whyItMatters: 'Blockchain metadata is immutable and transparent.',
            potentialLinkability: 'Heuristics evaluate correlation vectors.',
            limitations: 'Heuristics are probabilistic.',
            defensiveConsiderations: 'Use fresh addresses and coin control.',
          },
        });
      }
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Unknown AI error';
      const targetLabel = context?.targetId ? `${context.targetId.slice(0, 12)}...` : 'the requested entity';
      const score = context?.privacyAnalysis?.overallRiskScore ?? 58;
      const level = context?.privacyAnalysis?.riskLevel ?? 'MODERATE';
      ...
      const fallbackResponse = {
        ...
        whatIsVisible: `• Target: ${context?.targetId || 'Bitcoin ledger data'}\n• Inputs & Outputs: Public scripts, values in satoshis, and confirmation block height.\n• Fee telemetry: Recorded on public blockchain.`,
        whyItMatters: 'Public ledger data is permanent and transparent. Passive observers use heuristic clustering to link counterparties.',
        potentialLinkability: 'Heuristics like Common-Input-Ownership and differentiable change outputs indicate potential entity grouping.',
        limitations: 'DEFENSIVE SCOPE LIMITATION: All assessments are probabilistic heuristics and do not prove real-world identities or legal ownership.',
        defensiveConsiderations: '1. Practice single-use address discipline for every transaction.\n2. Utilize coin control to avoid clustering unrelated UTXOs.\n3. Adopt Taproot (P2TR) scripts for uniform on-chain appearance.',
      };
      return res.json({ structuredResponse: fallbackResponse, note: 'Resilient fallback generated', rawError: msg });
    }
  });

  // Blockchain node proxy routes to ensure CORS compatibility
  app.get('/api/blockchain/tx/:txid', async (req: Request, res: Response) => {
    const { txid } = req.params;
    try {
      const response = await fetch(`https://mempool.space/api/tx/${txid}`);
      if (!response.ok) {
        return res.status(response.status).json({ error: `Node error: ${response.statusText}` });
      }
  ...
  app.get('/api/blockchain/address/:address', async (req: Request, res: Response) => {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`BlockShield AI Server running on http://0.0.0.0:${PORT}`);
  });
}
